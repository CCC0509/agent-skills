...
git commit -m "test: add red review continuation smoke coverage"
```

Expected: commit succeeds with only `tests/install-smoke.sh` and this plan file changed. The commit message intentionally names the red state because the repository remains red until Task 2 updates and commits doctrine.
...
Do not run `tests/install-smoke.sh` yet expecting green in this pre-commit step:
that smoke clones committed `HEAD`, while the doctrine edits are still
...
If it exits nonzero, do not proceed to Task 3. Fix the doctrine, smoke token, or
metadata mismatch named by the failure, rerun `tests/install-smoke.sh`, rerun
`git diff --check`, then commit the focused follow-up:

```bash
git add tests/install-smoke.sh skills/agent-operating-manual/10-model-dispatch.md skills/multi-angle-review/SKILL.md ROADMAP.md .claude-plugin/plugin.json .claude-plugin/marketplace.json docs/superpowers/plans/2026-07-07-review-continuation-handoff-tightening.md
git commit -m "docs: fix review continuation install smoke"
```

Expected: any follow-up commit is narrow and exists only if the post-commit
install smoke exposed a mismatch.
...
fix-confirmation for this implementation plan:
...
Prev reviewed tip: <LAST_REVIEWED_TIP>
```

Expected: final handoff requests fresh full review, carries only the canonical ATK accepted residual, and points `Prev reviewed tip` at the reviewed implementation-plan tip rather than the earlier spec tip.
