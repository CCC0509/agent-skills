...
回覆 `Required user text` 指定的精確文字或裁決；`none` 表示不用再動。常見預設：
...
- [ ] **Step 4: Add consistency rules after the existing Relay readiness rule**

Insert this paragraph after the paragraph ending with `不可執行描述。`:

```md
User action consistency rule：`Status: complete-no-action-needed` 必須搭配
`User action: none`。`Review: none-FYI` 不能搭配含 `to-reviewer` 的 `User action`。
...
- Review feedback handling: The previous reviewer findings 1-7 are addressed, with one explicit disposition: F6 is tracked as a v0.4.9 landed entry rather than an Extraction Candidate because it is the current implementation scope.
