...
### §3.1 交接請求合約（Hand-off Request Contract）— moved

Split out for length (`40-maintenance.md` §4's ~250-line cap); no rule text
changed, only file location. Relay block fields, completion-report stage,
copy-block examples, `Target repo`/`Target`, and execution-surface /
capability-preflight mechanics now live in
[`11-relay-fields.md`](11-relay-fields.md) (§3.1 continues there). Status
semantics, relay readiness / consistency rules, escalation, execution
route, the pre-handoff self-check, and context-health / continuity /
provenance now live in [`12-relay-decisions.md`](12-relay-decisions.md).
Read both in order.
